...
export const GetAllComplaint = async (id) => {
  try {
    const response = await axios.get(
      `/complaints/get-all-complaint/${id}`,
      config
    );
...
export const handleupVote = async (payload) => {
  try {
    ...
    const response = await axios.put(
      `/complaints/vote/${payload.complaint._id}`,
      { id: payload.user._id },
      config
    );
...
export const handledownVote = async (payload) => {
  try {
    const response = await axios.put(
      `/complaints/unvote/${payload.complaint._id}`,
      { id: payload.user._id },
      config
    );
    return response.data;
...
export const handleDelete = async (payload) => {
  try {
    const response = await axios.delete(
      `/complaints/delete-complaint/${payload.complaint._id}`,
      {
