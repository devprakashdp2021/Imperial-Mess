...
const AddMessItem = asyncHandler(async (req, res) => {
  ...
  if (messItem) {
    ...
    return res
      .status(200)
      .send({ success: true, message: "Mess Item added successfully" });
  }
...
const RemoveMessItem = asyncHandler(async (req, res) => {
  try {
    ...
    if (!messItem) {
      return res
        .status(404)
        .send({ success: false, message: "Mess Item not found" });
    }
    if (messItem.information.length === 1) {
      await MessItem.deleteOne({ _id: itemId });
      return res
        .status(200)
        .send({ success: true, message: "Mess Item removed successfully" });
    } else {
